...
<osmand_routing_config defaultProfile="pedestrian-shade" name="shade-1">
    ...
    <routingProfile name="pedestrian-shade" baseProfile="pedestrian" restrictionsAware="false" minSpeed="2" defaultSpeed="4" maxSpeed="10" sharpTurn="0" slightTurn="0" followSpeedLimitations="false" onewayAware="false">
        ...
        <parameter id="avoid_stairs" name="Avoid stairs" description="Avoid stairs" type="boolean"/>
          <parameter id="avoid_test" name="Avoid test" description="Avoid test" type="boolean"/>
        """
xml_middle = """<parameter id="avoid_unpaved" name="Avoid unpaved roads" description="Avoid unpaved roads" type="boolean"/>
...
def priority_function(p):
    if p != 101:
        return 0.5 + 0.5 * p * 0.01
    return 0.5

def main():
    with open("shade-routing.xml", "w") as f:
        f.write(xml_deb)
        """<parameter id="shading_hour" name="Shading Hour" description="Hour for shading" type="numeric" values="0,8,10,12,14,16,18,20" valueDescriptions="night,8h,10h,12h,14h,16h,18h,20h"/>"""
        f.write("""<parameter id="shading_hour" name="Shading Hour" description="Hour for shading" type="numeric" values="0""")
        ...
        for hour in shade_hours:
            #f.write(f"<!--Shade for {hour} o'clock-->")
            f.write(f"""<eq value1=":shading_hour" value2="{hour}">""")
            for p in range (0, 102):
                f.write(f"""<select value="{priority_function(p)}" t="shade{hour}" v="{p}"/>""")
            f.write("</eq>")

        f.write(xml_end)

    parser = etree.XMLParser(remove_blank_text=True)
    tree = etree.parse("shade-routing.xml", parser)
    tree.write(
        "shade-routing.xml",
        pretty_print=True,
        encoding="utf-8",
